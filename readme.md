...
* `better-auth` - Authentication system (Frontend + Backend)
* `axios` - HTTPs request to endpoints
* `express.js` - Backend Logic + Endpoints
* `bullmq`   - Job queue system
* Docker - To use Redis for queued jobs (port: 6379, name: my-redis)
* `socket.io` - Real time communation for updates
...
* [X] Validation
* [X] Modeling Shapes
* [X] Socket.io for real time connections (might apply)
...
* [X] Status reconciliation (provider vs local)
* [ ] Stuck job detection
...
* [X] User registration (email) (better-auth)
* [ ] User regisration (OAuth) (better-auth)
* [X] Login / logout
* [X] Token strategy (JWT / session)
